...
module.exports = class MusicTriviaCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      aliases: ['music-quiz', 'start-quiz', 'musictrivia'],
      group: 'music',
      description: 'Quiz musicale!',
      guildOnly: true,
      ...
      args: [
        {
          key: 'numberOfSongs',
          prompt: 'Numero di canzoni che vuoi?',
          type: 'integer',
          default: 5,
          max: 15
        }
  ...
  async run(message, { numberOfSongs }) {
    ...
    if (!voiceChannel)
      return message.say('Devi essere in un canale plebeo');
    if (message.guild.musicData.isPlaying === true)
      return message.channel.send('Bruh stai riproducendo qualcosa, fermalo');
    ...
    const infoEmbed = new MessageEmbed()
      .setColor('#ff7373')
      .setTitle('Quiz di musica iniziato')
      .setDescription(
        `Ci so ${numberOfSongs} canzoni, hai 30 secondi per indovinare il nome della canzone oppure dell'autore. Hail 2 you!
        Puoi terminare la trivia con il comando m.stoptrivia`
      );
  ...
  static getLeaderBoard(arr) {
    ...
    leaderBoard = `👑   **${arr[0][0]}:** ${arr[0][1]}  punti`;

    if (arr.length > 1) {
      for (let i = 1; i < arr.length; i++) {
        leaderBoard =
          leaderBoard + `\n\n   ${i + 1}: ${arr[i][0]}: ${arr[i][1]}  punti`;
      }
    }
    return leaderBoard;
  }
