	...
	async obtain(msg, val, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			prompts.push(await msg.reply(stripIndents`
				${empty ? this.prompt : valid ? valid : `Hai dato un ${this.label} invalido. Prova di nuovo.`}
				${oneLine`
					Rispondi con \`cancel\` per cancellare il comando.
					${wait ? `Il comando verrà cancellato automaticamente in ${this.wait} secondi.` : ''}
				`}
			`));
